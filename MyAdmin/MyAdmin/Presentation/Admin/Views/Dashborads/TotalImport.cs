...
{
    public partial class TotalImport : UserControl
    {
        public TotalImport()
        {
            InitializeComponent();
            try
            {
                ...
                string sql = $"select SUM(amout) from import_prices v where MONTH(v.date) = {DateTime.Now.Month} and YEAR(v.date) = {DateTime.Now.Year} ;";
